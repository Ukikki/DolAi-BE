...
describe('parse RTP packet 1', () => {
    const view = (0, helpers_1.nodeBufferToDataView)(fs.readFileSync(path.join(__dirname, '..', '..', 'data', 'rtppacket1.raw')));
    let packet;
    test('isRtp() succeeds', () => {
        expect((0, RtpPacket_1.isRtp)(view)).toBe(true);
    });
    test('packet processing succeeds', () => {
        packet = new RtpPacket_1.RtpPacket(view);
        ...
        expect(packet.getExtension(1)).toEqual((0, helpers_1.numericArrayToDataView)([255]));
        expect(packet.needsSerialization()).toBe(false);
        packet.setExtension(1, (0, helpers_1.stringToDataView)('foo œæ€å∫∂ ®†¥∂ƒ∑©√'));
        expect(packet.getExtensions().size).toBe(1);
        expect(packet.needsSerialization()).toBe(true);
        expect(packet.getExtension(1)).toEqual((0, helpers_1.stringToDataView)('foo œæ€å∫∂ ®†¥∂ƒ∑©√'));
    });
});
describe('parse RTP packet 2', () => {
    const view = (0, helpers_1.nodeBufferToDataView)(fs.readFileSync(path.join(__dirname, '..', '..', 'data', 'rtppacket2.raw')));
    let packet;
    test('isRtp() succeeds', () => {
        expect((0, RtpPacket_1.isRtp)(view)).toBe(true);
    });
    test('packet processing succeeds', () => {
        ...
        expect(packet.getExtension(3))
            .toEqual((0, helpers_1.numericArrayToDataView)([0x65, 0x34, 0x1E]));
...
describe('parse RTP packet 4', () => {
    ...
    test('packet processing succeeds', () => {
        ...
        expect(packet.getExtension(2)).toEqual((0, helpers_1.numericArrayToDataView)([0x42]));
        expect(packet.getExtension(3)).toEqual((0, helpers_1.numericArrayToDataView)([0x11, 0x22]));
...
describe('parse RTP packet 5 with uncommon header extension value', () => {
    ...
    test('packet processing succeeds', () => {
        packet = new RtpPacket_1.RtpPacket(view);
        const packetView = (0, helpers_1.clone)(packet.getView());
        const payloadView = (0, helpers_1.clone)(packet.getPayload());
        const packetDump = (0, helpers_1.clone)(packet.dump());
        expect(packet.getByteLength()).toBe(array.byteLength);
        expect(packet.getPayloadType()).toBe(1);
        expect(packet.getSequenceNumber()).toBe(8);
        expect(packet.getTimestamp()).toBe(4);
        ...
        packet.serialize();
        expect((0, helpers_1.areDataViewsEqual)(packet.getView(), packetView)).toBe(true);
        expect((0, helpers_1.areDataViewsEqual)(packet.getPayload(), payloadView)).toBe(true);
...
describe('create RTP packet 6 from scratch', () => {
    let packet;
    test('packet processing succeeds', () => {
        packet = new RtpPacket_1.RtpPacket();
        expect((0, RtpPacket_1.isRtp)(packet.getView())).toBe(true);
        ...
        expect(packet.hasTwoBytesExtensions()).toBe(true);
        expect(packet.getExtensions().size).toBe(0);
        packet.setExtension(1, (0, helpers_1.stringToDataView)('foo'));
        expect(packet.needsSerialization()).toBe(true);
        expect(packet.getExtensions().size).toBe(1);
        expect(packet.getExtension(1)).toEqual((0, helpers_1.stringToDataView)('foo'));
        packet.setExtension(2, (0, helpers_1.numericArrayToDataView)([1, 2, 3, 4]));
        expect(packet.getExtensions().size).toBe(2);
        expect(packet.needsSerialization()).toBe(true);
        expect(packet.getExtension(2)).toEqual((0, helpers_1.numericArrayToDataView)([1, 2, 3, 4]));
        ...
        expect(packet.getExtensions().size).toBe(1);
        expect(packet.getExtension(2)).toEqual((0, helpers_1.numericArrayToDataView)([1, 2, 3, 4]));
        packet.setPayload((0, helpers_1.stringToDataView)('codec'));
        expect(packet.needsSerialization()).toBe(true);
        expect(packet.getPayload()).toEqual((0, helpers_1.stringToDataView)('codec'));
    });
    test('packet.clone() succeeds', () => {
        ...
        expect(clonedPacket.getExtension(2))
            .toEqual((0, helpers_1.numericArrayToDataView)([1, 2, 3, 4]));
        expect(clonedPacket.getPayload()).toEqual((0, helpers_1.stringToDataView)('codec'));
        ...
        expect((0, helpers_1.areDataViewsEqual)(clonedPacket.getPayload(), packet.getPayload())).toBe(true);
        ...
        expect(clonedPacket.getPayload().buffer === packet.getPayload().buffer).toBe(false);
        ...
        expect(clonedPacket2.getPayload().buffer === packet.getPayload().buffer).toBe(false);
    });
    test('packet.rtxEncode() and packet.rtxDecode() succeed', () => {
        ...
        const payloadWithoutSeqNumberView = new DataView(packet.getPayload().buffer, packet.getPayload().byteOffset + 2, packet.getPayload().byteLength - 2);
        expect((0, helpers_1.areDataViewsEqual)(payloadWithoutSeqNumberView, payloadView)).toBe(true);
...
describe('create RTP packet 7 from scratch', () => {
    ...
    test('packet processing succeeds', () => {
        ...
        packet.setExtension(1, (0, helpers_1.numericArrayToDataView)([1, 2, 3, 4]));
        expect(packet.needsSerialization()).toBe(true);
        expect(packet.getExtensions().size).toBe(1);
        expect(packet.getExtension(1)).toEqual((0, helpers_1.numericArrayToDataView)([1, 2, 3, 4]));
        expect(packet.hasOneByteExtensions()).toBe(true);
        expect(packet.hasTwoBytesExtensions()).toBe(false);
...
describe('create RTP packet 8 from scratch', () => {
    ...
    test('packet processing succeeds', () => {
        ...
        expect(packet.needsSerialization()).toBe(true);
        expect(packet.getExtensions().size).toBe(0);
        packet.setExtension(0, (0, helpers_1.stringToDataView)('ignore me'));
        packet.setExtension(1, (0, helpers_1.numericArrayToDataView)([1, 2, 3, 4]));
        packet.setExtension(15, (0, helpers_1.stringToDataView)('also ignore me'));
        packet.setExtension(16, (0, helpers_1.stringToDataView)('also ignore me'));
...
describe('create RTP packet 9 from scratch', () => {
    ...
    test('packet processing succeeds', () => {
        ...
        packet.setExtension(0, (0, helpers_1.stringToDataView)('ignore me'));
        packet.setExtension(1, (0, helpers_1.numericArrayToDataView)([1, 2, 3, 4]));
        packet.setExtension(256, (0, helpers_1.stringToDataView)('also ignore me'));
        packet.setExtension(257, (0, helpers_1.stringToDataView)('also ignore me'));
        // We don't validate extensions (`id` and value length) until the packet
        // is serialized.
        expect(packet.getExtensions().size).toBe(4);
...
describe('create RTP packet 10 from scratch', () => {
    ...
    test('packet processing succeeds', () => {
        packet = new RtpPacket_1.RtpPacket();
        packet.setPayload((0, helpers_1.numericArrayToDataView)([1, 2, 3, 4]));
        expect(packet.needsSerialization()).toBe(true);
        ...
        expect(packet.getByteLength()).toBe(16);
        expect(packet.getPadding()).toBe(0);
        packet.setPayload((0, helpers_1.numericArrayToDataView)([1, 2, 3, 4, 5]));
...
describe('clone packet into a given buffer', () => {
    ...
    test('cloning succeeds', () => {
        ...
        // Packet and payload views must be the same.
        expect((0, helpers_1.areDataViewsEqual)(clonedPacket.getView(), packetView)).toBe(true);
        expect((0, helpers_1.areDataViewsEqual)(clonedPacket.getPayload(), payloadView)).toBe(true);
